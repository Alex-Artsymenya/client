...
public class AddAppointmentController implements Initializable {
    ...
    public void OnMouseClickedServices() throws IOException {
        ...
        List<EmployeeService> employees = new Gson().fromJson(batchGetService.GetEntities(Requests.GET_ALL_EMPLSERV),
                                                              listType);
        ...
        if (employees.size() != 0) {
            for (EmployeeService employeeService : employees) {
                if (employeeService.getService().getId() == servId) {
                    EmployeeModel tableEmployee = new EmployeeModel(employeeService.getEmployee().getId(),
                                                                    employeeService.getEmployee().getName(),
                                                                    employeeService.getEmployee().getSurname(),
                                                                    employeeService.getEmployee().getServices()
                                                                                   .toString());
    ...
    public void onSave() throws ParseException, IOException, InterruptedException {
        ...
        try {
            int servId = serviceTable.getSelectionModel().getSelectedItem().getId();
            List<EmployeeService> employees = new Gson().fromJson(
                    batchGetService.GetEntities(Requests.GET_ALL_EMPLSERV), listType);
            ...
            for (EmployeeService employeeService1 : employees) {
                if (employeeService1.getEmployee().getId() == employee.getId()
                    && employeeService1.getService().getId() == service.getId()) {
                    employeeService.setId(employeeService1.getId());
                }
            }

            Appointment appointment = new Appointment();
            System.out.println(java.sql.Date.valueOf(dateChoice.getValue().toString()).toString());
        ...
        if (clients.size() != 0) {
            for (Client client : clients) {
                if (client.getUser().getId() == ClientSocket.getInstance().getUser().getId()) {
                    ClientModel tableUser = new ClientModel(client.getId(),
            ...
            if (list2.size() == 0) {
                ...
                List<Service> services = new Gson().fromJson(batchGetService.GetEntities(Requests.GET_ALL_SERVICES),
                                                             listType);
